...
class RemindMe(Base):
    ...
    def remindme(self, update: Update, context: CallbackContext) -> None:
        ...
        try:
            ...
            context.job_queue.run_once(
                self.alarm, delta.total_seconds(), context={"chat_id": chat_id, "message_id": message_id}, name="{}_{}".format(chat_id, message_id)
            )

            update.message.reply_text("I will remind you this {} ({})!".format(self._naturaltime(delta), str(delta).split(".")[0]))
    ...
    def allremindme(self, update: Update, context: CallbackContext) -> None:
        ...
        if update.message.chat.type == CHAT_SUPERGROUP:
            ...
            liste = [
                "- #{}: {}.".format(i, str(job.next_t).split(".")[0])
